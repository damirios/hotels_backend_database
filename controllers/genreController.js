...
class GenreController {
    async createGenre(req, res) {
        ...
        const newGenre = await db.query("INSERT into genre_simple (title) values ($1) RETURNING *", 
        [title]);
    ...
    async updateGenre(req, res) {
        ...
        let updatedGenre;
        if (id) {
            const genre = await db.query("SELECT * FROM genre_simple WHERE genre_id = $1", [id]);
            if (title) {
                updatedGenre = await db.query('UPDATE genre_simple set title = $1 WHERE genre_id = $2 RETURNING *', 
                [title, id]);
    ...
    async deleteGenre(req, res) {
        const id = req.params.id;
        if (id) {
            await film_genre_controller.deleteGenreRecords(id);
            const genre = await db.query(`DELETE FROM genre_simple WHERE genre_id = $1  RETURNING *`, [id]);
            res.send(genre.rows[0]);
        } else {
